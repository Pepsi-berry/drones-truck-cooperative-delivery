...
class UpperSolverTrainingEnvironment():
    ...
    metadata = {
        ...
        "name": "upper_training_environment_v3",
    }
    ...
    def get_range_mask_by_uav_type(self, cust_pos_both, truck_pos, uav_type):
        dist = np.linalg.norm(cust_pos_both - truck_pos, axis=1)
        
        return (dist <= 0.5 * self.uav_range[uav_type]).astype(int)
    
    
    def greedy_uav_TA_policy(self, avail_uav, cust_pos_both, truck_pos, mask, mask_uav_0, mask_uav_1):
        avail_uav_type = 0 if avail_uav < self.num_uavs_0 else 1
        avail_uav_dist = np.linalg.norm(cust_pos_both - truck_pos, axis=1)
        # avail_uav_mask = np.where(avail_uav_dist < self.uav_range[avail_uav_type]*0.5, 1, 0)
        if avail_uav_type == 0:
            avail_uav_mask = mask_uav_0 * mask[self.num_customer_truck:]
            if self.mask_range:
                avail_uav_mask = avail_uav_mask * self.get_range_mask_by_uav_type(cust_pos_both, truck_pos, 0)
        else:
            avail_uav_mask = mask_uav_1 * mask[self.num_customer_truck:]
            if self.mask_range:
                avail_uav_mask = avail_uav_mask * self.get_range_mask_by_uav_type(cust_pos_both, truck_pos, 1)
        
        avail_uav_dist = avail_uav_dist * avail_uav_mask 
        
        if np.all(avail_uav_dist == 0):
            return None
        else:
            return np.argmax(avail_uav_dist)
    ...
    def step(self, action):
        ...
        for i in range(self.batch_size):
            ...
            if self.termination[i]:
                continue
            avai_carrier = self.available_carrier[i]
            mask_uav_0 = self.mask_uav_0[i]
            mask_uav_1 = self.mask_uav_1[i]
            mask = self.mask[i]
            ...
            # break the loop when the truck completed the delivery task, which means the truck is available to be assigned task
            completed = False
            while not completed:
                completed = False
                # truck moves
                ...
                if completed: 
                    break
                avail_uav_list = np.flatnonzero(avai_carrier[1:])
                avail_uav = avail_uav_list[-1] if avail_uav_list.size > 0 else None
                if avail_uav is not None:
                    # at least one uav available to be assigned task
                    # assign the furthest task from the truck for available uav
                    avail_uav_task = self.greedy_uav_TA_policy(avail_uav, cust_pos_both, truck_pos, mask, mask_uav_0, mask_uav_1)
                    if avail_uav_task is not None: # empty uav action
                        self.last[i] = np.int64(avail_uav_task + self.num_customer_truck + 1)
                        uav_stage[avail_uav] = 1
                        avai_carrier[avail_uav + 1] = 0
...
def env_creator(env_config={}):
    env = UpperSolverTrainingEnvironment(
        ...
        uav_velocity=env_config.get("uav_velocity", np.array([12, 20])), 
